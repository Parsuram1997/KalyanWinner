...
export default function EnrollerWithdrawPage() {
  ...
  useEffect(() => {
    if (commissionBalance > 0) {
      form.setValue("amount", commissionBalance);
    }
  }, [commissionBalance, form]);
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    if (values.amount > commissionBalance) {
        form.setError("amount", { type: "manual", message: "Withdrawal amount cannot exceed your bonus balance." });
        return;
    }
    ...
    try {
      const result = await createTransaction({
        ...
        type: "Withdrawal",
        status: "Pending",
        description: `Enroller withdrawal request to ${userData.paymentMethod === 'bank' ? userData.accountNumber : userData.upiId}`,
      });
  ...
  const defaultWhatsappMessage = "Hello, I need help regarding my enroller withdrawal.";
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-xl">Request Bonus Withdrawal</CardTitle>
        <CardDescription>You can withdraw funds from your commission balance.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="p-4 rounded-lg bg-muted text-center">
            ...
            {isLoading ? 
                ...
                <p className="text-2xl font-bold font-mono">₹{commissionBalance.toFixed(0)}</p>
            }
        </div>
        ...
        {isLoading ? <Skeleton className="h-24 w-full" /> : !hasPaymentDetails ? (
            <Alert variant="destructive">
              ...
              <Button asChild className="mt-4" size="sm">
                  <Link href="/enroller/wallet/account">Add Payment Details</Link>
              </Button>
            </Alert>
        ) : (
          <div className="mb-4">
             <Card>
                <CardHeader className="flex flex-row items-center justify-between pb-2">
                    ...
                    <Button variant="outline" size="sm" asChild>
                       <Link href="/enroller/wallet/account">Change</Link>
                    </Button>
                </CardHeader>
